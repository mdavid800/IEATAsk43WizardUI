...
function CalibrationArray({
  ...
  const { fields: calibrationFields, append: appendCalibrationRHF, remove: removeCalibrationRHF } = useFieldArray({
    control,
    name: `measurement_location.0.sensor.${sensorIndex}.calibration`
  });
        ...
        <div key={calField.id} className="border rounded-lg overflow-hidden">
          ...
          {expandedCalibrations[calField.id] && (
            <div className="p-6 space-y-6 bg-background">
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor={`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.measurement_type_id`}>
                    Measurement Type
                  </Label>
                  <Select
                    onValueChange={(value) => setValue(`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.measurement_type_id`, value as MeasurementType)}
                    value={watch(`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.measurement_type_id`)}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Select measurement type" />
                    </SelectTrigger>
                ...
                <div className="space-y-2">
                  <Label htmlFor={`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.slope`}>
                    Slope
                  </Label>
                  <Input
                    type="number"
                    step="any" // Allow more precision
                    {...register(`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.slope`, { valueAsNumber: true })}
                    placeholder="Enter slope value"
                  />
                </div>

                <div className="space-y-2">
                  <Label htmlFor={`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.offset`}>
                    Offset
                  </Label>
                  <Input
                    type="number"
                    step="any"
                    {...register(`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.offset`, { valueAsNumber: true })}
                    placeholder="Enter offset value"
                  />
                </div>

                <div className="space-y-2">
                  <Label htmlFor={`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.sensitivity`}>
                    Sensitivity
                  </Label>
                  <Input
                    type="number"
                    step="any"
                    {...register(`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.sensitivity`, { valueAsNumber: true })}
                    placeholder="Enter sensitivity value"
                  />
                </div>

                <div className="space-y-2">
                  <Label htmlFor={`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.calibration_id`}>
                    Calibration ID
                  </Label>
                  <Input
                    {...register(`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.calibration_id`)}
                    placeholder="Enter calibration ID"
                  />
                </div>

                <div className="space-y-2">
                  <Label htmlFor={`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.date_of_calibration`}>
                    Calibration Date
                  </Label>
                  <Input
                    type="date"
                    {...register(`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.date_of_calibration`)}
                  />
                </div>

                <div className="space-y-2">
                  <Label htmlFor={`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.calibration_organisation`}>
                    Calibration Organisation
                  </Label>
                  <Input
                    {...register(`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.calibration_organisation`)}
                    placeholder="Enter organisation name"
                  />
                </div>

                <div className="space-y-2">
                  <Label htmlFor={`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.place_of_calibration`}>
                    Place of Calibration
                  </Label>
                  <Input
                    {...register(`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.place_of_calibration`)}
                    placeholder="Enter calibration location"
                  />
                </div>

                <div className="space-y-2">
                  <Label htmlFor={`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.uncertainty_k_factor`}>
                    Uncertainty K Factor
                  </Label>
                  <Input
                    type="number"
                    step="any"
                    {...register(`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.uncertainty_k_factor`, { valueAsNumber: true })}
                    placeholder="Enter k factor"
                  />
                </div>

                <div className="space-y-2">
                  <Label htmlFor={`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.revision`}>
                    Revision
                  </Label>
                  <Input
                    {...register(`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.revision`)}
                    placeholder="e.g., 2.0 or B"
                  />
                </div>

                <div className="space-y-2">
                  <Label htmlFor={`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.report_file_name`}>
                    Report File Name
                  </Label>
                  <Input
                    {...register(`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.report_file_name`)}
                    placeholder="e.g., calibration_report.pdf"
                  />
                </div>

                <div className="space-y-2">
                  <Label htmlFor={`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.report_link`}>
                    Report Link
                  </Label>
                  <Input
                    type="url"
                    {...register(`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.report_link`)}
                    placeholder="Enter URL to calibration report"
                  />
                </div>

                <div className="sm:col-span-2 space-y-2">
                  <Label htmlFor={`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.notes`}>
                    Notes
                  </Label>
                  <Textarea
                    {...register(`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.notes`)}
                    placeholder="Add any additional notes"
                    rows={3}
                  />
                </div>
...
function UncertaintyArray({
  ...
  const { fields: uncertaintyFields, append: appendUncertainty, remove: removeUncertainty } = useFieldArray({
    control,
    name: `measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.calibration_uncertainty`
  });
      ...
      <div className="space-y-4">
        {uncertaintyFields.map((uncField, uncIndex) => (
          <div key={uncField.id} className="grid grid-cols-1 gap-4 sm:grid-cols-3 p-4 bg-muted/50 rounded-lg relative">
            ...
            <div className="space-y-2">
              <Label htmlFor={`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.calibration_uncertainty.${uncIndex}.reference_bin`}>
                Reference Bin
              </Label>
              <Input
                type="number"
                step="any"
                {...register(`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.calibration_uncertainty.${uncIndex}.reference_bin`, { valueAsNumber: true })}
                placeholder="Enter reference bin"
              />
            </div>

            <div className="space-y-2">
              <Label htmlFor={`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.calibration_uncertainty.${uncIndex}.reference_unit`}>
                Reference Unit
              </Label>
              <Input
                {...register(`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.calibration_uncertainty.${uncIndex}.reference_unit`)}
                placeholder="e.g., m/s"
              />
            </div>

            <div className="space-y-2">
              <Label htmlFor={`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.calibration_uncertainty.${uncIndex}.combined_uncertainty`}>
                Combined Uncertainty
              </Label>
              <Input
                type="number"
                step="any"
                {...register(`measurement_location.0.sensor.${sensorIndex}.calibration.${calIndex}.calibration_uncertainty.${uncIndex}.combined_uncertainty`, { valueAsNumber: true })}
                placeholder="Enter combined uncertainty"
              />
            </div>
          </div>
